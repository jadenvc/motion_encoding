...
def main():
    latent_path = "/home/erikrozi/LaneGCN/results/train_latents_epoch0.npy"
    h2o.init()
    seed = hvd.rank()
    ...
    remaining_epochs = 10#int(np.ceil(config["num_epochs"] - epoch))
    ...
    if not latent_path:
        ...
        isolation_model = output['isolation_model']
        training_data = output['training_data']
    else:
        training_data = np.load(latent_path, allow_pickle=True)
    ...
    preds = isolation_model.predict(training_data)
    
    # Identify anomolies
    bounds = 5.5
    anomolies = (preds['mean_length']< bounds).as_data_frame().to_numpy().flatten().nonzero()[0]
    non_anomolies = (preds['mean_length']>=bounds).as_data_frame().to_numpy().flatten().nonzero()[0]
